...
int main() {
    ...
    // Forzar ejecución sólo del Escenario 1
    int scenario = 1;
    current_scenario = 1;
    printf("\n✓ Escenario fijado a 1: P1 → P2 → P3 (Modo restringido)\n");
    
    // Forzar uso del archivo de temperaturas 1 únicamente
    int temp_file_choice = 1;
    char filename[30];
    sprintf(filename, "temperaturas%d.txt", temp_file_choice);
    printf("\n✓ Archivo de temperaturas fijado a: %s\n", filename);
